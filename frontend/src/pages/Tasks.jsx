      ...
      <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-6">
        <div>
           <div className="flex items-center gap-2 mb-1">
              <span className="text-[10px] font-bold text-primary-600 uppercase tracking-[0.3em] bg-primary-50 px-2 py-0.5 rounded-md border border-primary-100">Task Matrix</span>
              <span className="w-1.5 h-1.5 rounded-full bg-emerald-500 animate-pulse"></span>
           </div>
           <h1 className="text-4xl font-display font-bold text-slate-900 tracking-tighter uppercase">Objectives</h1>
         ...
         <div className="md:col-span-2 bg-white rounded-[2.5rem] border border-slate-100 p-8 shadow-soft flex flex-col justify-between">
            <div>
               <div className="flex items-center justify-between mb-4">
                  <h3 className="text-sm font-bold text-slate-800 uppercase tracking-widest">Completion Index</h3>
            ...
            <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mt-6">System Efficiency: Optimal</p>
         </div>
      ...
      <AnimatePresence>
        {showForm && (
          ...
          >
            ...
            <h3 className="text-2xl font-bold text-slate-900 mb-8 tracking-tight flex items-center gap-3 relative z-10">
            ...
            <form onSubmit={handleSubmit} className="space-y-8 relative z-10">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                ...
                <div className="space-y-2">
                  <label className="text-[10px] font-bold text-slate-400 uppercase tracking-widest px-1">Priority Protocol</label>
                ...
                <div className="space-y-2">
                  <label className="text-[10px] font-bold text-slate-400 uppercase tracking-widest px-1">Execution Date</label>
                ...
                <div className="md:col-span-2 space-y-2">
                  <label className="text-[10px] font-bold text-slate-400 uppercase tracking-widest px-1">Assigned Profile</label>
              ...
              <div className="mt-10 flex gap-4">
                <button type="submit" disabled={saving} className="flex-1 btn-primary py-4 rounded-2xl font-bold text-xs uppercase tracking-widest shadow-xl shadow-primary-500/25">{saving ? 'Executing...' : 'Finalize Task'}</button>
                <button type="button" onClick={() => setShowForm(false)} className="px-10 py-4 bg-slate-50 text-slate-500 font-bold text-xs uppercase tracking-widest rounded-2xl hover:bg-slate-100 transition-all border border-slate-100">Cancel</button>
              </div>
            </form>
          </motion.div>
        )}
      ...
      <div className="space-y-12">
        {pendingTasks.length > 0 && (
          <div className="space-y-6">
            <h3 className="text-xs font-bold text-slate-400 uppercase tracking-[0.3em] px-2 flex items-center gap-2">
               Active Objectives
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-3">
                        <p className="text-base font-bold text-slate-800 truncate">{task.title}</p>
                        <span className={`px-2 py-0.5 rounded text-[8px] font-bold uppercase tracking-widest border ${meta.bg} ${meta.color} ${meta.border}`}>{task.priority}</span>
                      </div>
                      <div className="flex items-center gap-4 text-[10px] font-bold text-slate-400 mt-1 uppercase tracking-tighter">
                        ...
                        {task.pet && <span className="flex items-center gap-1.5 text-primary-600 font-bold"><Footprints className="w-3.5 h-3.5" /> {task.pet.name}</span>}
                      </div>
                    </div>
        ...
        {completedTasks.length > 0 && (
          <div className="space-y-6">
            <h3 className="text-xs font-bold text-slate-400 uppercase tracking-[0.3em] px-2 flex items-center gap-2 opacity-50">
               Archived Success
            </h3>
        ...
        {!loading && safeTasks.length === 0 && (
          <div className="py-32 bg-white rounded-[3rem] border-2 border-dashed border-slate-100 flex flex-col items-center justify-center text-center space-y-4">
             ...
             <div>
                <h3 className="text-xl font-bold text-slate-900">Zero Objectives</h3>
                <p className="text-slate-400 text-sm font-medium">All systems operational. No pending tasks detected.</p>
             </div>
             <button onClick={() => setShowForm(true)} className="text-primary-600 font-bold text-xs uppercase tracking-widest hover:underline">Manual Initialization</button>
          </div>
        )}
      </div>
    </div>
